...
static void drv8848_assert_io_config(void) {
    ...
    struct io_config curr_motor_enable_config, curr_motor_right_nfault_config, curr_motor_left_nfault_config;
    ...
    ASSERT(io_config_compare(&motor_nfault_config, &curr_motor_right_nfault_config));
    ASSERT(io_config_compare(&motor_nfault_config, &curr_motor_left_nfault_config));
}
